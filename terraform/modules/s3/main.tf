...
resource "aws_kms_key" "s3" {
  description             = "KMS key for IAM-Secure-Gate S3 bucket encryption"
  ...
  tags = merge(var.common_tags, {
    Name    = "${local.bucket_prefix}-s3-kms"
    Service = "KMS"
  })
}
...
resource "aws_s3_bucket" "logs" {
  bucket = "${local.bucket_prefix}-logs-${local.account_id}"
